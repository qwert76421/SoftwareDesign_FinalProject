...
public abstract class RegularLevel extends Level {
	...
	protected void createMobs() {
		...
		int mobsToSpawn = RegularLevelMobSpawnRules.mobsToSpawn(Dungeon.depth, mobLimit());
		...
		// enemies cannot be within a 8-tile FOV or 8-tile open space walk from the
		// entrance
		...
		// doors within the entrance room are ignored for this walk, but doors on the
		// edge are not
		...
		while (mobsToSpawn > 0) {
			...
			} while (tries >= 0 && RegularLevelMobSpawnRules.shouldRetryPlacement(
	...
	protected void createItems() {
		...
		for (int i = 0; i < nItems; i++) {
			...
			// if (Dungeon.depth > 1 && Random.Float() < mimicChance && findMob(cell) ==
			// null) {
			...
			// }
			// }
			// 先存起來：避免重複呼叫（也讓規則輸入更清楚）
			float mimicMultiplier = MimicTooth.mimicChanceMultiplier();
			boolean cellHasMob = findMob(cell) != null;

			// 1) Base container/mimic 決策（保留原本 Random 呼叫次數）
			// Random.Int(20) 一定會抽
			int roll20 = Random.Int(20);

			// 原碼只有 roll=1..4 時才會抽 Random.Float()
			float chestMimicRoll = Float.NaN;
			if (roll20 >= 1 && roll20 <= 4) {
				chestMimicRoll = Random.Float();
			}

			RegularLevelItemSpawnRules.BaseDropDecision baseDecision = RegularLevelItemSpawnRules.decideBaseDrop(
					roll20,
					Dungeon.depth,
					cellHasMob,
					mimicMultiplier,
			...
			if (baseDecision.spawnNormalMimic) {
				mobs.add(Mimic.spawnAt(cell, toDrop));
				continue;
			}

			Heap.Type type = baseDecision.heapTypeIfNotMimic;

			// 2) Special drop 觸發判斷：保留原本短路 Random.Int(...) 的行為
			boolean specialTriggered = false;

			if (toDrop instanceof Artifact) {
				specialTriggered = (Random.Int(2) == 0);
			}

			if (!specialTriggered && toDrop.isUpgradable()) {
				specialTriggered = (Random.Int(4 - toDrop.level()) == 0);
			}

			// 3) Special drop 決策：Golden Mimic vs Locked Chest + Key
			if (specialTriggered) {

				// 原碼只有 depth>1 才會抽 Random.Float()（因為 if 的第一個 &&）
				float goldenRoll = Float.NaN;
				if (Dungeon.depth > 1) {
					goldenRoll = Random.Float();
				}

				RegularLevelItemSpawnRules.SpecialOutcome special = RegularLevelItemSpawnRules.decideSpecialDrop(
						true,
						Dungeon.depth,
						cellHasMob,
						mimicMultiplier,
				...
				if (special == RegularLevelItemSpawnRules.SpecialOutcome.GOLDEN_MIMIC) {
					mobs.add(Mimic.spawnAt(cell, GoldenMimic.class, toDrop));
				} else {
					Heap dropped = drop(toDrop, cell);
					if (heaps.get(cell) == dropped) {
		...
		// use separate generator(s) for this to prevent held items, meta progress, and
		// talents from affecting levelgen
		...
		// we increment dropped by 2 for compatibility with old saves, when the talent
		// dropped 4/6 items
		Random.pushGenerator(Random.Long());
		if (Dungeon.hero.hasTalent(Talent.CACHED_RATIONS)) {
			...
			if (Dungeon.depth >= targetFloor
					&& dropped.count() < 2 + 2 * Dungeon.hero.pointsInTalent(Talent.CACHED_RATIONS)) {
				int cell;
				int tries = 100;
				boolean valid;
				do {
		...
		// a total of 6 pages drop randomly, the rest are specially dropped or are given
		// at the start
		...
		// lore pages
		// TODO a fair bit going on here, I might want to refactor/externalize this in
		// the future
		Random.pushGenerator(Random.Long());
	...
	private static LevelExplorePercentRules.HeapType toRuleHeapType(Heap.Type type) {
		if (type == Heap.Type.HEAP)
			return LevelExplorePercentRules.HeapType.HEAP;
		if (type == Heap.Type.FOR_SALE)
			return LevelExplorePercentRules.HeapType.FOR_SALE;
		if (type == Heap.Type.CRYSTAL_CHEST)
			return LevelExplorePercentRules.HeapType.CRYSTAL_CHEST;
		return LevelExplorePercentRules.HeapType.OTHER;
	}

	private static boolean heapContainsKey(Heap h) {
		for (Item i : h.items) {
			if (i instanceof Key)
				return true;
		}
	...
	public float levelExplorePercent(int depth) {
		...
		// There are levelgen heaps which are undiscovered, in an openable container, or
		// which contain keys
		// for (Heap h : heaps.valueList()) {
		// if (h.autoExplored)
		// continue;
		...
		// }
		for (Heap h : heaps.valueList()) {
			boolean missed = LevelExplorePercentRules.heapMarksRoomMissed(
					h.autoExplored,
					h.seen,
					toRuleHeapType(h.type),
					heapContainsKey(h));
		...
		// There is magical fire (blocks items) or sacrificial fire (contains items) in
		// it
		...
		// There are unused crystal keys for this room (only one crystal key room can be
		// on each floor)
		// we ignore regular and golden keys as earlier checks would have already caught
		// them
		...
		// Note that it is NOT required for the player to see every tile or discover
		// every trap.
		...
		// return 0f;
		// }
		return LevelExplorePercentRules.scoreFromMissedRoomsCount(missedRooms.size());
