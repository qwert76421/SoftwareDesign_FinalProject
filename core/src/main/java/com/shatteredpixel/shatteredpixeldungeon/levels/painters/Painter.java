...
public abstract class Painter {
	...
	// Painters take a level and its collection of rooms, and paint all the specific
	// tile values
	public abstract boolean paint(Level level, ArrayList<Room> rooms);

	public static void set(PaintTarget t, int x, int y) {
		set(t.level, x, y, t.value); // 委派回原本方法，行為不變
	}

	public static void fill(PaintTarget t, int x, int y, int w, int h) {
		fill(t.level, x, y, w, h, t.value);
	}

	public static void fill(PaintTarget t, Rect rect) {
		fill(t.level, rect, t.value);
	}

	public static void drawLine(PaintTarget t, Point from, Point to) {
		drawLine(t.level, from, to, t.value);
	}
	...
	public static void fillDiamond(Level level, int x, int y, int w, int h, int value) {
		...
		// we want the end width to be w, and the width will grow by a total of (h-2 -
		// h%2)
