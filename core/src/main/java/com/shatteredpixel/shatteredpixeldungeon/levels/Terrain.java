...
public class Terrain {
	...
	public static final int CUSTOM_DECO_EMPTY = 32; // regular empty tile that can't be overridden, used for custom
													// visuals mainly
	// solid environment decorations
	public static final int CUSTOM_DECO = 23; // invisible decoration that will also be a custom visual, re-uses the old
												// terrain ID for signs
	...
	private static final int TERRAIN_COUNT = 256;
	public static final int[] flags = new int[TERRAIN_COUNT];
	...
	private static final int[] DISCOVER_TO = new int[TERRAIN_COUNT];

	static {
		java.util.Arrays.fill(DISCOVER_TO, -1);
		DISCOVER_TO[SECRET_DOOR] = DOOR;
		DISCOVER_TO[SECRET_TRAP] = TRAP;
	}

	public static int discover(int terr) {
		// 保持原本 switch 的 default 行為：不認識就回傳原值
		// 同時確保超出範圍不會 array out of bounds

		if (terr < 0 || terr >= TERRAIN_COUNT)
			return terr;
		int mapped = DISCOVER_TO[terr];
		return mapped != -1 ? mapped : terr;
	}
