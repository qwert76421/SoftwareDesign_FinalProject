...
public class Hero extends Char {
	...
	public void updateHT(boolean boostHP) {
		...
		// 1) 純公式：等級與 HTBoost 得到 baseHT
		int base = HeroVitalsRules.baseHT(lvl, HTBoost);

		// 2) 外部依賴：戒指倍率由遊戲系統提供（仍在 Hero 取得）
		float multiplier = RingOfMight.HTMultiplier(this);
		int newHT = HeroVitalsRules.applyMultiplier(base, multiplier);

		// 3) 外部依賴：Elixir buff 的 boost 數值（仍在 Hero 取得）
		ElixirOfMight.HTBoost elixir = buff(ElixirOfMight.HTBoost.class);
		if (elixir != null) {
			newHT = HeroVitalsRules.applyElixirBoost(newHT, elixir.boost());
		}

		// 4) 純規則：依 boostHP 決定 HP 如何跟著變
		int newHP = HeroVitalsRules.newHP(oldHP, oldHT, newHT, boostHP);
	...
	public int STR() {
		...
		int ringBonus = RingOfMight.strengthBonus(this);

		int adrenalineBonus = 0;
		AdrenalineSurge adrenaline = buff(AdrenalineSurge.class);
		if (adrenaline != null) {
			adrenalineBonus = adrenaline.boost();
		}

		int strongmanBonus = 0;
		if (hasTalent(Talent.STRONGMAN)) {
			strongmanBonus = HeroStrRules.strongmanBonus(baseSTR, pointsInTalent(Talent.STRONGMAN));
		}

		return HeroStrRules.totalSTR(baseSTR, ringBonus, adrenalineBonus, strongmanBonus);
	}
	...
	private boolean getCloser(final int target) {
		...
		if (step != -1) {
			...
			if (Dungeon.level.pit[step] && !Dungeon.level.solid[step]
					&& (!flying
							|| buff(Levitation.class) != null && buff(Levitation.class).detachesWithinDelay(delay))) {
	...
	public void earnExp(int exp, Class source) {
		...
		int threshold = HeroProgressionRules.maxExpForLevel(lvl);
		float percent = exp / (float) threshold;

		HeroExpGainEffects.apply(this, percent, exp, source);
		...
		while (this.exp >= threshold) {
			...
			if (lvl < MAX_LEVEL) {
				...
				// 升級後門檻會變，更新 threshold（對應原本 maxExp() 會隨 lvl 變）
				threshold = HeroProgressionRules.maxExpForLevel(lvl);
	...
	public int maxExp() {
		// return maxExp( lvl );
		return HeroProgressionRules.maxExpForLevel(lvl);
	}
